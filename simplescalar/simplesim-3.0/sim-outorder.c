 ...
 */
 ...
 /* MLP stats (file-scope globals) */
     extern unsigned long mlp_miss_count;
extern unsigned long long mlp_sum_fixed;
 


                                     /* (store mlp_score * 1e6 as integer) */
#define MLP_SCALE 1000000.0            /* scale factor for fixed-point */
...
sim_reg_stats(struct stat_sdb_t *sdb)   /* stats database */
{
  ...
  stat_reg_counter(sdb, "mlp_miss_count",
      "Number of misses with recorded MLP",
      &mlp_miss_count, 0, NULL);
      
    stat_reg_counter(sdb, "mlp_sum_fixed",
      "Scaled sum of MLP scores (mlp_score * 1e6)",
      &mlp_sum_fixed, 0, NULL);    

  /* formula: average MLP = mlp_sum_fixed / (MLP_SCALE * mlp_miss_count)
     Note: stat formulas accept floating operations, so divide by 1000000.0 */
  stat_reg_formula(sdb, "avg_mlp",
      "Average MLP score (mlp_sum_fixed / 1e6 / mlp_miss_count)",
      "mlp_sum_fixed / (1000000.0 * mlp_miss_count)", NULL);
