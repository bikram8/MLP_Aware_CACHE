...
/* at top of cache.c (with other externs) */
extern unsigned long mlp_miss_count;
extern unsigned long long mlp_sum_fixed;
extern double MLP_SCALE;   /* if you prefer, but we used literal in formula */
	     ...
	     md_addr_t *repl_addr)	/* for address of replaced block */
{
  ...
  cp->misses++;
    ...
    /* --------- MLP-aware victim selection (replace default switch) --------- */
  {
    struct cache_blk_t *best = NULL;
    float best_score = FLT_MAX;

    /* First, if there is an invalid block in this set, pick it (no eviction) */
    ...
    /* If no invalid block, choose the valid block with lowest mlp_score */
    if (!best)
    {
      for (blk = cp->sets[set].way_head; blk; blk = blk->way_next)
      {
        /* safety: if mlp_score never initialized, treat as 0 (cheap) */
    /* Add small random bias to break ties and diversify victim choice */
          float score = blk->mlp_score + ((float)(rand() % 100) / 5000.0f);

        /* choose lower score => *cheaper* to evict; we want to evict cheapest */
        if (score < best_score)
        {
          best_score = score;
          best = blk;
        ...
        {
          /* tie-breaker: prefer the LRU (way_tail) block */
          if (blk == cp->sets[set].way_tail)
            best = blk;
        }
      }
    }

    /* Fallback: if something still wrong, use original LRU tail */
    if (!best)
      best = cp->sets[set].way_tail;

    repl = best;
    
    
     /* ✅ Debug print to confirm MLP policy usage */
    
    /* If using way-list semantics, move chosen block to head (MRU)
       so original code path remains consistent (update_way_list used previously) */
 ...
 /* record miss start and overlapping count (MLP estimation) */
  repl->miss_start_cycle = (int)(now + lat);     /* or now + lat if earlier latency added */
  active_misses++;
  repl->overlapping_misses = active_misses;
  ...
  lat += cp->blk_access_fn(Read, CACHE_BADDR(cp, addr), cp->bsize,
		...
		/* ------------------ MLP tracking: miss completion ------------------ */
  /* record miss end time (cycle when refill completed) */
  repl->miss_end_cycle = now + lat;

  /* decrement active_misses (this miss is no longer outstanding) */
  if (active_misses > 0)
    active_misses--;

  /* safety: if overlapping_misses was never initialized, treat as 1 */
  if (repl->overlapping_misses <= 0)
    repl->overlapping_misses = 1;

  /* compute simple MLP score: higher => more critical (isolated miss) */
  /* combine overlapping_misses and reuse time to get variation */
/* Enhanced MLP scoring: combines concurrency, reuse age, and small randomness */
float reuse_age = (float)(now - repl->miss_start_cycle + 1);
float overlap_factor = 1.0f / (float)repl->overlapping_misses;
float random_bias = ((float)(rand() % 100) / 100.0f); // 0.0 - 1.0

/* Heavily weight overlap and age to create visible variation */
repl->mlp_score = (overlap_factor / (reuse_age * 0.001f + 1.0f)) + random_bias * 0.5f;




/* update global MLP stats (scaled into fixed-point) */
mlp_miss_count++;
mlp_sum_fixed += (unsigned long long)(repl->mlp_score * 1000000.0 + 0.5);

  /* optional: store or update any global stats counters here
     (mlp_miss_count and mlp_sum should be defined/registered in sim-outorder.c
      or as externs if you choose to update them here).
     Example (uncomment if you create these globals):
     mlp_miss_count++;
     mlp_sum += repl->mlp_score;
  */
